...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $mysqli->prepare("UPDATE `addstudent` SET 
        `imageOfStudent` = IFNULL(?, `imageOfStudent`), `academicYear` = ?, `admissionDate` = ?, `rollNumber` = ?, 
        `studentStatus` = ?, `fnameOfStudent` = ?, `lnameOfStudent` = ?, `class` = ?, 
        `section` = ?, `gender` = ?, `dateOfBirth` = ?, `bloodGroup` = ?, `house` = ?, 
        `religion` = ?, `feesGroup` = ?, `caste` = ?, `primaryContact` = ?, 
        `emailOfStudent` = ?, `motherTongue` = ?, `languageKnown` = ?, 
        `imageOfFather` = IFNULL(?, `imageOfFather`), `fatherName` = ?, `emailOfFather` = ?, `fatherContact` = ?, 
        `fatherProfession` = ?, `imageOfMother` = IFNULL(?, `imageOfMother`), `motherName` = ?, 
        `emailOfMother` = ?, `motherContact` = ?, `motherProfession` = ?, 
        `guardianName` = ?, `guardianRelation` = ?, `guardianContact` = ?, 
        `guardianEmail` = ?, `guardianOccupation` = ?, `guardianAddress` = ?, 
        `imageOfGuardian` = IFNULL(?, `imageOfGuardian`), `currentAddressOfStudent` = ?, 
        `permanentAddressOfStudent` = ?, `districtOfStudent` = ?, 
        `provinceOfStudent` = ?, `transportRoute` = ?, `vehicleNumber` = ?, 
        `pickUpPoint` = ?, `hostel` = ?, `hostelRoomNumber` = ?, 
        `documentOfBirthCertificate` = IFNULL(?, `documentOfBirthCertificate`), `documentOfTransferCertificate` = IFNULL(?, `documentOfTransferCertificate`), 
        `allergiesOfStudent` = ?, `medicationOfStudent` = ?, `previousSchoolName` = ?, 
        `previousSchoolAddress` = ?, `bankName` = ?, `branchOfBank` = ?, 
        `ifscNumber` = ?, `otherInfo` = ? WHERE `admissionNumber` = ?");
    ...
    // Handle image uploads (same function as before)
    ...
    if ($stmt->bind_param(
        ...
        $ifscNumber,
        $otherInfo,
        $admission_number // Bind the admission number for the WHERE clause
    )) {
        $stmt->execute();

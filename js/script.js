$(document).ready(function () {
    ...
    if ($('.toggle-passworda').length > 0) {
        $(document).on('click', '.toggle-passworda', function () {
            $(this).toggleClass("ti-eye ti-eye-off"); var input = $(".pass-inputa"); if (input.attr("type") == "password") { input.attr("type", "text"); } else {
                setTimeout
...
$(document).ready(function () {
    ...
    setTimeout(function () { $(".edit-popup").on("click", function () { $('.toggle-popup1').addClass('sidebar-popup'); }); $(".sidebar-close1").on("click", function () { $('.toggle-popup1').removeClass('sidebar-popup'); }); }, 100); if ($('.select-people-checkbox').length > 0) {
        $(document).on('click', '.select-people-checkbox', function () {
            $(this).toggleClass('checkbox-checked'); $(this).find("input").attr('checked', 'checked'); if ($(this).hasClass('checkbox-checked')) { $(this).find("input").attr('checked', 'checked'); }
    ...
    $(".addsibling-info").on('click', '.trash-icon', function () { $(this).closest('.sibling-cont').remove(); return false; }); $(".add-sibling").on('click', function () {
        var servicecontent = '<div class="row sibling-cont">' +
            ...
            '</div>' +
            '</div>' +
            '</div>'; setTimeout(function () { $('.select'); setTimeout(function () { $('.select').select2({ minimumResultsForSearch: -1, width: '100%' }); }, 100); }, 100); $(".addsibling-info").append(servicecontent); return false;
    }); $(".promote-students-btn").on("click", function () { $(".promote-card-main").addClass("promote-card-main-show"); }); $(".reset-promote").on("click", function () { $(".promote-card-main").removeClass("promote-card-main-show"); }); if ($('.routine-slider').length > 0) { $('.routine-slider').owlCarousel({ items: 4, loop: true, margin: 15, nav: false, dots: false, autoplay: false, smartSpeed: 1000, responsive: { 0: { items: 1 }, 576: { items: 2 }, 768: { items: 3 }, 1200: { items: 2 }, 1300: { items: 3 }, 1400: { items: 4 } } }) }
    ...
    $(".add-new-timetable").on('click', function () {
        var timetablecontent = '<div class="row timetable-count">' +
            ...
            '</div>' +
            '</div>' +
            '</div>'; setTimeout(function () { $('.select'); setTimeout(function () { $('.select').select2({ minimumResultsForSearch: -1, width: '100%' }); }, 100); }, 100); $(".add-timetable-row").append(timetablecontent); return false;
    }); $(".add-timetable-row").on('click', '.delete-time-table', function () { $(this).closest('.timetable-count').remove(); return false; }); $(".add-new-schedule").on('click', function () {
    ...
    $(function () {
        $(".circle-progress").each(function () {
            var value = $(this).attr('data-value'); var left = $(this).find('.progress-left .progress-bar'); var right = $(this).find('.progress-right .progress-bar'); if (value > 0) {
